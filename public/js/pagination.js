...
var Pagination = /** @class */ (function () {
    ...
    Pagination.prototype.init = function (options) {
        ...
        (_a = document.querySelector(this.options.element)) === null || _a === void 0 ? void 0 : _a.innerHTML = "";
        // 渲染
        this.render();
    };
    // 渲染
    Pagination.prototype.render = function (flag) {
        var _this_1 = this;
        // 切换每页显示条数重新替换每页条数参数
        if (this.options.layout.indexOf("sizes") !== -1 &&
        ...
        // 单页隐藏
        if (this.pageNum === 1 &&
            this.options.singlePageHide &&
            !this.options.layout.split(",").some(function (v) { return v.trim() === "sizes"; })) {
            // 清空元素
        ...
        if (old_container) {
            ...
            this.options.layout.split(",").forEach(function (v) {
                var _a;
                if (Object.keys(params_1).includes(v.trim())) {
                    (_a = old_container === null || old_container === void 0 ? void 0 : old_container.querySelector("." + params_1[v.trim()])) === null || _a === void 0 ? void 0 : _a.replaceWith(_this_1[v.trim()]());
                }
            });
        }
        else {
            // 主体容器
            var container_1 = this.createElement("div", "_page_container");
            // layout 渲染
            this.options.layout.split(",").forEach(function (v) {
                if (_this_1._layout.indexOf(v.trim()) !== -1) {
                    element = _this_1[v.trim()]();
                    element && container_1.appendChild(element);
                }
            });
            // 保存元素
            this.element.appendChild(container_1);
        }
    };
    // 首页
    Pagination.prototype.home = function () {
        ...
        var element = this.createElement("div", __spreadArrays([
            "_home",
            "_home_" + this.options.type
        ], disabled));
    ...
    Pagination.prototype.prev = function () {
        ...
        var element = this.createElement("div", __spreadArrays([
            "_prev",
            "_prev_" + this.options.type
        ], disabled, (this.options.prevText ? ["_prev_text"] : ["_iconfont", "iconzuo"])));
    ...
    Pagination.prototype.pager = function () {
        ...
        arrs.forEach(function (v, k) {
            active =
                v === _this_1.options.pageIndex ? ["_active_" + _this_1.options.type] : [];
            // 手势禁止
            if (v === _this_1.options.pageIndex && _this_1.options.disabled)
                active.push("_disabled");
            li = _this_1.createElement("li", __spreadArrays([
                "_pages_li_" + _this_1.options.type
            ], active));
            if (isNaN(v)) {
                if (k <= 1) {
                    attr = "prev";
    ...
    Pagination.prototype.next = function () {
        ...
        var element = this.createElement("div", __spreadArrays([
            "_next",
            "_next_" + this.options.type
        ], disabled, (this.options.nextText ? ["_next_text"] : ["_iconfont", "iconGroup-"])));
        // 下一页文字
    ...
    Pagination.prototype.sizes = function () {
        ...
        var text = this.createElement("div", [
            "_sizes_text",
            this.uniqid + "_sizes_text",
        ]);
        var span = this.createElement("span", this.uniqid + "_sizes_text_content");
        span.innerText = this.options.pageSizes[this.selectedIndex] + "\u6761/\u9875";
                ...
                if (!isNaN(v)) {
                    ...
                    lis.addEventListener("click", function () {
                        if (_this.selectedIndex !== key) {
                            ul.querySelectorAll("._sizes_select_li").forEach(function (v, k) {
                                if (key === k) {
                                    v.classList.add("_sizes_select_active");
                                }
                                else {
                                    v.classList.remove("_sizes_select_active");
                                }
                            });
                            span.innerText = _this.options.pageSizes[key] + "\u6761/\u9875";
        ...
        var toggle = function () {
            text.classList.add("_sizes_active");
        ...
        var close = function (e) {
            if (!e.target.classList.contains(_this_1.uniqid + "_sizes_text") &&
                !e.target.classList.contains(_this_1.uniqid + "_icon_down") &&
                !e.target.classList.contains(_this_1.uniqid + "_sizes_text_content")) {
                _this_1.showSelector = true;
                toggle();
            }
        };
        // 显示选择框、旋转icon
    ...
    Pagination.prototype.total = function () {
        ...
        element.innerText = "total " + this.options.total + " page";
        return element;
    };
    // 页码变化
